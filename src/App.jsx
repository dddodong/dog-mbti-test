...
const HelpTooltip = ({ text }) => {
  ...
  return (
    <div className="relative inline-block ml-2">
      ...
      {visible && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-gray-800 text-white text-sm rounded-lg p-3 shadow-lg z-10 animate-fade-in-fast">
          {text}
        </div>
      )}
    </div>
...
const mbtiQuestions = [
    { id: 1, type: 'E/I', question: "딩동! 산책 갈 준비 됐냥? 우리 강아지는 문소리만 나도...", options: [{ text: "현관에서 미리 기다리며 '빨리 나가자!' 온몸으로 표현해요.", type: "E" }, { text: "주인이 산책줄을 들 때까지 조용히 기다리거나, 부르면 그제야 고개를 들어요.", type: "I" }], tooltip: "강아지가 외부 자극에 얼마나 즉각적이고 활발하게 반응하는지 확인하는 질문입니다." },
    { id: 2, type: 'E/I', question: "새로운 강아지 친구를 만났을 때, 우리 강아지의 첫 반응은?", options: [{ text: "일단 꼬리를 흔들며 다가가서 냄새 맡고 놀자고 재롱을 부려요.", type: "E" }, { text: "멀리서 지켜보다가 상대방이 다가오면 조심스럽게 탐색해요.", type: "I" }], tooltip: "사회적 상황에서 에너지를 얻는지(외향적), 아니면 신중하게 관찰하며 에너지를 쓰는지(내향적) 알아봅니다." },
    { id: 3, type: 'E/I', question: "혼자 집에 있을 때, 우리 강아지는?", options: [{ text: "가족이 없어서 외로워하고, 문 앞에서 기다리거나 하울링을 하기도 해요.", type: "E" }, { text: "혼자만의 시간을 즐기며 편안하게 쉬거나 잠을 자요.", type: "I" }], tooltip: "혼자 있는 시간에 대한 반응을 통해 사회적 상호작용의 필요성 정도를 파악합니다." },
    { id: 4, type: 'S/N', question: "주인이 간식 봉지를 열 때, 우리 강아지의 눈빛은?", options: [{ text: "간식 봉지의 '실물'에 눈을 고정하고, 냄새와 모양에 집중해요.", type: "S" }, { text: "봉지 소리만으로 이미 머릿속에 간식 파티가 펼쳐지는 듯 상상력이 풍부해요.", type: "N" }], tooltip: "정보를 받아들일 때, 오감으로 확인된 '사실'에 집중하는지, 아니면 소리 같은 단서로 '가능성'을 상상하는지 알아봅니다." },
    { id: 5, type: 'S/N', question: "처음 가는 놀이터에 도착! 우리 강아지는...", options: [{ text: "바닥 냄새부터 킁킁 맡으며 실시간으로 주변 환경을 파악해요.", type: "S" }, { text: "전체적인 분위기를 훑어보고, '어디서 신나게 뛰어볼까?' 큰 그림을 그려요.", type: "N" }], tooltip: "새로운 환경을 탐색할 때, 구체적인 사실(냄새)부터 파악하는지, 전체적인 맥락과 가능성을 먼저 보는지 확인합니다." },
    { id: 6, type: 'S/N', question: "변화가 생겼을 때 우리 강아지는?", options: [{ text: "기존 방식을 유지하려 하거나 익숙한 것에 안정을 찾아요.", type: "S" }, { text: "새로운 가능성을 탐색하거나 새로운 것에 호기심을 보여요.", type: "N" }], tooltip: "예상치 못한 변화에 대해, 익숙하고 현실적인 것을 선호하는지, 새로운 가능성에 더 흥미를 느끼는지 알아봅니다." },
    { id: 7, type: 'T/F', question: "주인이 슬퍼 보일 때, 우리 강아지의 행동은?", options: [{ text: "주인의 슬픔을 '문제'로 인식하고 해결하려는 듯 장난감을 가져다주거나 짖어요.", type: "T" }, { text: "제 감정을 그대로 읽고 옆에 와서 위로해주거나 핥아줘요.", type: "F" }], tooltip: "결정을 내리거나 반응할 때, 객관적인 해결책을 찾으려 하는지(사고형), 아니면 감정적인 공감과 조화를 우선하는지(감정형) 봅니다." },
    { id: 8, type: 'T/F', question: "다른 강아지와 싸움이 났을 때, 우리 강아지는?", options: [{ text: "옳고 그름을 따지듯, 자신의 영역이나 권리를 주장하며 단호하게 대처해요.", type: "T" }, { text: "상대방의 기분이나 분위기를 살피며 싸움을 피하려고 하거나, 빨리 화해하려 해요.", type: "F" }], tooltip: "갈등 상황에서 논리적 원칙에 따라 행동하는지, 관계의 조화를 유지하려 하는지 파악합니다." },
    { id: 9, type: 'T/F', question: "주인이 꾸중할 때, 우리 강아지는?", options: [{ text: "왜 꾸중을 듣는지 '논리적' 이유를 파악하려 하고, 다음에는 어떻게 해야 할지 생각해요.", type: "T" }, { text: "주인의 표정이나 목소리 톤에 따라 감정적으로 풀이 죽거나 불안해해요.", type: "F" }], tooltip: "부정적인 피드백에 대해, 원인 분석에 집중하는지, 감정적으로 먼저 반응하는지 알아봅니다." },
    { id: 10, type: 'J/P', question: "하루 일과 중 '밥 먹는 시간'에 대한 우리 강아지의 생각은?", options: [{ text: "규칙적인 시간에 맞춰 '밥 시간 됐잖아!' 시계를 보는 것처럼 정확해요.", type: "J" }, { text: "그때그때 상황에 따라 배고프면 먹고, 아니면 나중에 먹어도 상관없어요.", type: "P" }], tooltip: "생활 방식이 계획적이고 체계적인지(판단형), 아니면 유연하고 즉흥적인지(인식형) 알아봅니다." },
    { id: 11, type: 'J/P', question: "새로운 장소로 이사했을 때, 우리 강아지는?", options: [{ text: "적응하는 데 시간이 걸리지만, 꾸준히 탐색하며 새 집의 규칙을 익혀나가요.", type: "J" }, { text: "새로운 환경에 대한 호기심이 많고, 바로 새로운 곳을 자유롭게 돌아다니며 탐험해요.", type: "P" }], tooltip: "새로운 환경에 적응할 때, 체계적으로 파악하려 하는지, 아니면 자유롭게 탐색하며 적응하는지 확인합니다." },
    { id: 12, type: 'J/P', question: "놀이 시간이 끝나고, 우리 강아지는?", options: [{ text: "놀았던 장난감을 스스로 정리하거나, 놀이 공간을 정돈하려는 모습을 보여요.", type: "J" }, { text: "놀이가 끝나면 바로 다른 일에 몰두하고, 장난감은 그 자리에 그냥 둬요.", type: "P" }], tooltip: "활동을 마무리하는 방식을 통해, 정리정돈과 마무리를 중시하는지, 아니면 유연하게 다음 활동으로 넘어가는지 봅니다." }
];
...
function App() {
  // State Management
  const [page, setPage] = useState('start'); // start, mbti, result, survey, complete, admin
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'admin': return <AdminPage onExit={() => { setIsAdmin(false); setPage('start'); }} />;
  ...
  const MbtiTestPage = () => {
    ...
    return (
      <div className="animate-fade-in">
        ...
        <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6 text-center h-24 flex items-center justify-center">
          {question.question}
  ...
  const ResultPage = () => {
    ...
    const resultData = mbtiResults[mbtiResult.type];
    ...
    return (
      <div className="text-center animate-fade-in">
        <div className="bg-white p-6 rounded-lg">
          ...
          <p className="text-xl sm:text-2xl font-semibold text-gray-800 mb-4 whitespace-nowrap">{resultData.nickname}</p>
          ...
          <div className="bg-purple-50 p-6 rounded-xl text-left mb-6 shadow-inner space-y-3">
             <p><strong>한 줄 요약:</strong> {resultData.summary}</p>
        ...
        <div className="my-6 flex justify-center items-center space-x-2 sm:space-x-3">
            <button onClick={() => handleManualShare('인스타그램')} className="p-2 bg-gradient-to-br from-yellow-400 via-red-500 to-purple-600 text-white rounded-full shadow-md hover:opacity-90 transition-opacity" title="인스타그램에 공유">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
            </button>
            <button onClick={() => handleManualShare('틱톡')} className="p-2 bg-black text-white rounded-full shadow-md hover:opacity-90 transition-opacity flex items-center justify-center" title="틱톡에 공유">
            ...
            <button onClick={() => shareOnSns('naver')} className="p-2 bg-[#03C75A] text-white rounded-full shadow-md hover:opacity-90 transition-opacity flex items-center justify-center" title="네이버 블로그에 공유">
        ...
        <p className="text-lg text-gray-700 mb-6 font-semibold bg-yellow-100 border-l-4 border-yellow-400 p-4 rounded-r-lg">
          <span className="text-purple-600 font-bold">[{mbtiResult.type}]</span> 특성을 가진 우리 강아지, 어떤 맞춤형 케어가 필요할까요? <br/>우리 강아지 특성에 맞는 맞춤형 돌봄케어를 원하시는 분만 답해주세요.
        </p>
              ...
              {q.type === 'checkbox' && (
                <div className="space-y-3">
                  {q.options.map((opt, idx) => (
                    <label key={idx} className="flex items-center text-gray-700 cursor-pointer p-2 rounded-md hover:bg-gray-100">
                  ...
                  {q.hasOther && (
                    <div className="flex items-center mt-3">
                      <label className="flex items-center text-gray-700 cursor-pointer p-2 rounded-md hover:bg-gray-100">
  ...
  const AdminPage = ({ onExit }) => {
    ...
    const [filter, setFilter] = useState({ keyword: '', mbti: 'all', sort: 'newest' });
    ...
    const exportToCsv = () => {
        ...
        filteredSubmissions.forEach(s => {
            const row = [
                ...
                `"${s.surveyResponses.q2_newServiceIdeas || ''}"`,
                `"${s.surveyResponses.q3_previousServiceFeedback || ''}"`,
            ...
            <div className="bg-gray-50 p-6 rounded-xl shadow-md">
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                    {filteredSubmissions.map(s => (
                        <div key={s.submissionId} className="bg-white p-4 rounded-lg shadow">
